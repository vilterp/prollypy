...
impl Repo {
    ...
    pub fn get_head(&self) -> StoreResult<(Option<Commit>, String)> {
        let current_ref = match self.commit_graph_store.get_head()? {
        ...
        let commit = match self.commit_graph_store.get_ref(&current_ref)? {
            Some(hash) => self.commit_graph_store.get_commit(&hash)?,
            None => None,
        };

        Ok((commit, current_ref))
    }
    ...
    pub fn create_branch(&self, name: &str, from_commit: Option<&Hash>) -> StoreResult<()> {
        // Check if branch already exists
        if self.commit_graph_store.get_ref(name)?.is_some() {
            return Err(StoreError::Other(format!("Branch '{}' already exists", name)));
        }
        ...
        let commit_hash = if let Some(hash) = from_commit {
            ...
            if let Some(commit) = head_commit {
                ...
                return Err(StoreError::Other("Cannot create branch: no commits exist yet".to_string()));
            }
        };

        // Verify the commit exists
        if self.commit_graph_store.get_commit(&commit_hash)?.is_none() {
            return Err(StoreError::NotFound(format!("Commit {} does not exist", hex::encode(&commit_hash))));
        }
    ...
    pub fn checkout(&self, ref_name: &str) -> StoreResult<()> {
        if self.commit_graph_store.get_ref(ref_name)?.is_none() {
            return Err(StoreError::NotFound(format!("Ref '{}' does not exist", ref_name)));
        }
    ...
    pub fn get_commit(&self, commit_hash: &Hash) -> StoreResult<Option<Commit>> {
        self.commit_graph_store.get_commit(commit_hash)
    }

    /// List all branches and their commit hashes.
    pub fn list_branches(&self) -> StoreResult<Vec<(String, Hash)>> {
        Ok(self.commit_graph_store.list_refs()?
            .into_iter()
            .collect())
    }
    ...
    pub fn resolve_ref(&self, ref_str: &str) -> StoreResult<Option<Hash>> {
        ...
        if ref_str.starts_with("HEAD~") {
            ...
            // Start with HEAD
            let (head_commit, _) = self.get_head()?;
            let mut current_commit_hash = match head_commit {
                Some(c) => c.compute_hash(),
                None => return Ok(None),
            };

            // Walk back n parents
            for _ in 0..n {
                let current_commit = match self.get_commit(&current_commit_hash)? {
                    Some(c) => c,
                    None => return Ok(None),
                };
                if current_commit.parents.is_empty() {
                    return Ok(None); // No parent available
                }
                // Take the first parent
                current_commit_hash = current_commit.parents[0].clone();
            }

            return Ok(Some(current_commit_hash));
        }

        // Handle HEAD specially
        if ref_str == "HEAD" {
            let (head_commit, _) = self.get_head()?;
            return Ok(head_commit.map(|c| c.compute_hash()));
        }
    ...
    pub fn walk(&self, frontier: &HashSet<Hash>) -> StoreResult<Vec<(Hash, Commit)>> {
    ...
    ///
    /// Vector of (commit_hash, commit) tuples in reverse chronological order
    pub fn log(&self, start_ref: Option<&str>, max_count: Option<usize>) -> StoreResult<Vec<(Hash, Commit)>> {
    ...
    pub fn get_reachable_tree_roots(&self) -> StoreResult<HashSet<Hash>> {
    ...
    fn collect_node_hashes(&self, tree_root: &Hash) -> StoreResult<HashSet<Hash>> {
    ...
    pub fn get_nodes_to_push(&self, base_commit: Option<&Hash>) -> StoreResult<HashSet<Hash>> {
        // Get HEAD commit
        let (head_commit, _) = self.get_head()?;
    ...
    pub fn push<R: Remote + 'static>(
        ...
        let (done_tx, done_rx): (Sender<Result<Hash, StoreError>>, Receiver<Result<Hash, StoreError>>) = channel::unbounded();
        ...
        remote.update_ref(&ref_name, remote_commit_hex.as_deref(), &head_hash_hex)?;
    ...
    pub fn pull<R: Remote + 'static>(
        ...
        let remote_commit_hash = hex::decode(&remote_commit_hex)?;
        ...
        let have_locally = {
            ...
            move |item_type: PullItemType, item_hash: &Hash| -> bool {
                match item_type {
                    PullItemType::Commit => commit_graph_store.get_commit(item_hash).map(|o| o.is_some()).unwrap_or(false),
                    PullItemType::Node => block_store.get_node(item_hash).map(|o| o.is_some()).unwrap_or(false),
                }
            }
        };
            ...
            let handle = thread::spawn(move || {
                loop {
                    ...
                    match item_type {
                        PullItemType::Commit => {
                            // Download commit
                            if let Ok(Some(commit)) = remote.get_commit(&item_hash) {
                                {
                        ...
                        PullItemType::Node => {
                            // Download node
                            if let Ok(Some(node)) = remote.get_node(&item_hash) {
                                let _ = block_store.put_node(&item_hash, (*node).clone());
...
mod tests {
    ...
    fn test_init_empty() {
        ...
        let repo = Repo::init_empty(block_store.clone(), commit_store.clone(), "test@example.com".to_string()).unwrap();

        // Should have a main branch
        let refs = repo.list_branches().unwrap();
        assert_eq!(refs.len(), 1);
        assert_eq!(refs[0].0, "main");

        // HEAD should point to main
        let (commit, ref_name) = repo.get_head().unwrap();
    ...
    fn test_create_branch() {
        let block_store = Arc::new(MemoryBlockStore::new());
        let commit_store = Arc::new(MemoryCommitGraphStore::new());
        let repo = Repo::init_empty(block_store, commit_store, "test@example.com".to_string()).unwrap();
        ...
        let main_hash = repo.resolve_ref("main").unwrap().unwrap();
        let develop_hash = repo.resolve_ref("develop").unwrap().unwrap();
    ...
    fn test_checkout() {
        ...
        let (_, ref_name) = repo.get_head().unwrap();
    ...
    fn test_commit() {
        let block_store = Arc::new(MemoryBlockStore::new());
        let commit_store = Arc::new(MemoryCommitGraphStore::new());
        let repo = Repo::init_empty(block_store.clone(), commit_store, "test@example.com".to_string()).unwrap();
        ...
        // Create a commit
        let commit = repo.commit(&new_root, "Test commit", None, None, None).unwrap();
    ...
    fn test_resolve_ref() {
        ...
        // Resolve HEAD
        let head_hash = repo.resolve_ref("HEAD").unwrap().unwrap();
        let main_hash = repo.resolve_ref("main").unwrap().unwrap();
        ...
        tree.insert_batch(vec![(b"key".to_vec(), b"value".to_vec())], false);
        let new_root = tree.get_root_hash();
        repo.commit(&new_root, "Second commit", None, None, None).unwrap();

        // Resolve HEAD~
        let parent_hash = repo.resolve_ref("HEAD~").unwrap().unwrap();
        assert_eq!(parent_hash, main_hash);

        // Resolve HEAD~1 (same as HEAD~)
        let parent_hash_1 = repo.resolve_ref("HEAD~1").unwrap().unwrap();
    ...
    fn test_get_reachable_tree_roots() {
        ...
        let (initial_commit, _) = repo.get_head().unwrap();
        ...
        repo.create_branch("develop", Some(&repo.resolve_ref("HEAD~1").unwrap().unwrap())).unwrap();
        repo.checkout("develop").unwrap();
        let mut tree = ProllyTree::new(0.01, 42, Some(block_store.clone()));
        tree.insert_batch(vec![(b"branch_key".to_vec(), b"value".to_vec())], false);
        let branch_root = tree.get_root_hash();
        repo.commit(&branch_root, "Branch commit", None, None, None).unwrap();

        // Get reachable tree roots
        let reachable = repo.get_reachable_tree_roots().unwrap();
