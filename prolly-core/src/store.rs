...
impl BlockStore for MemoryBlockStore {
    ...
    fn get_node(&self, node_hash: &Hash) -> StoreResult<Option<Arc<Node>>> {
        ...
        Ok(nodes.get(node_hash).cloned())
    }

    fn delete_node(&self, node_hash: &Hash) -> StoreResult<bool> {
        let mut nodes = self.nodes.lock()?;
        Ok(nodes.remove(node_hash).is_some())
    }
...
impl FileSystemBlockStore {
    ...
    fn serialize_node(&self, node: &Node) -> StoreResult<Vec<u8>> {
        bincode::serialize(node).map_err(StoreError::from)
    }

    /// Deserialize a node from bincode.
    fn deserialize_node(&self, data: &[u8]) -> StoreResult<Node> {
    ...
    fn list_nodes(&self) -> StoreResult<Vec<Hash>> {
        ...
        for entry in entries {
            ...
            if subdir_path.is_dir() {
                let subdir_entries = fs::read_dir(&subdir_path)?;
...
impl BlockStore for CachedFSBlockStore {
    ...
    fn get_node(&self, node_hash: &Hash) -> StoreResult<Option<Arc<Node>>> {
        ...
        let node = match self.fs_store.get_node(node_hash)? {
            Some(n) => n,
            None => return Ok(None),
        };
...
) -> StoreResult<Box<dyn BlockStore>> {
    if spec == ":memory:" {
        ...
        Err(StoreError::Other("S3 storage not yet implemented".to_string()))
    } else {
        Err(StoreError::Other(format!("Invalid store spec: {}", spec)))
    }
}
...
pub trait Remote: BlockStore {
    ...
    /// Returns Ok(()) if update succeeded, Err(RefConflict) if there was a conflict.
    fn update_ref(&self, ref_name: &str, old_hash: Option<&str>, new_hash: &str) -> StoreResult<()>;

    /// Store a commit by its hash
    fn put_commit(&self, commit_hash: &Hash, commit: crate::commit_graph_store::Commit) -> StoreResult<()>;

    /// Retrieve a commit by its hash. Returns None if not found.
    fn get_commit(&self, commit_hash: &Hash) -> StoreResult<Option<crate::commit_graph_store::Commit>>;
...
mod tests {
    ...
    fn test_memory_store() {
        ...
        store.put_node(&hash, node.clone()).unwrap();
        ...
        let deleted = store.delete_node(&hash).unwrap();
        assert!(deleted);
    ...
    fn test_cached_store() {
        ...
        // Put both nodes
        store.put_node(&hash1, node1.clone()).unwrap();
        store.put_node(&hash2, node2.clone()).unwrap();

        // Get node1 - should be cache hit
        let _ = store.get_node(&hash1).unwrap();
        let stats = store.get_cache_stats();
        assert!(stats.cache_hits > 0);

        // Get non-existent node - should be cache miss
        let _ = store.get_node(&vec![99, 99, 99]).unwrap();
        let stats = store.get_cache_stats();
    ...
    fn test_lru_eviction() {
        ...
        // Fill cache
        store.put_node(&hash1, node1).unwrap();
        store.put_node(&hash2, node2).unwrap();

        // Access hash1 to make it recently used
        let _ = store.get_node(&hash1).unwrap();

        // Add hash3 - should evict hash2 (least recently used)
        store.put_node(&hash3, node3).unwrap();
    ...
    fn test_create_store_from_spec() {
        ...
        let node = Node::new_leaf();
        let hash = vec![1, 2, 3];
        store.put_node(&hash, node).unwrap();
        assert_eq!(store.count_nodes().unwrap(), 1);
    }
}
